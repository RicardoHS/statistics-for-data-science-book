...
\[ \mu_{X} = \mathbb{E}[X] = \sum_{x} x\,P(X=x) \]

if $p(x)$ is not an accurate characterization, then,
\[ \mu_{X} \approx \mathbb{E}[X] \]
...
\[ \sigma^2_X = V[X] = \mathbb{E}[(X-\mu)^2] = \mathbb{E}[X^2] - \mu^2 = \sum_{x}(x-\mathbb{E}[X])^2 P(X=x) \]

if $p(x)$ is not an accurate characterization, then,
\[ \sigma^2_{X} \approx  V[X] \]
